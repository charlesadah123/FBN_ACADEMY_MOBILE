...
class RecordSettingFireRepo implements RecordSettingRepo {
  ...
  DatabaseReference db_RecordSettings =
      FirebaseDatabase.instance.ref(DbPaths.recordSetting);

  @override
  Future<void> createRecordSetting(RecordSetting recordSetting) async {
    if (user != null) {
      await db_RecordSettings
          .child(recordSetting.id)
          .set(recordSetting.toJson());
  ...
  Future<RecordSetting?> getRecordSetting() async {
    if (user != null) {
      ...
      RecordSetting rec =
          RecordSetting.fromJson(snapshot.value as Map<dynamic, dynamic>);
