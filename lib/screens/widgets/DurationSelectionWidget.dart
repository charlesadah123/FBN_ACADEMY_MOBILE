...
class DurationSelectionWidget extends StatefulWidget {
  ...
  const DurationSelectionWidget({Key? key, required this.onDurationSelected}) : super(key: key);
       ...
       // Example fill color
        ...
        onChanged: (int? newValue) {
          if (newValue != null) {
            ...
            widget.onDurationSelected(selectedDuration); // Pass the difference as Duration
          }
        },
        items: durations.map<DropdownMenuItem<int>>((int duration) {
          final startDate = DateTime.now().subtract(Duration(days: duration));
          final displayText = '${startDate.day} ${DateFormat.MMM().format(startDate)}  ($duration Days)';
